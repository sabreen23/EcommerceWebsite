...
class APPMAKER_WC_YITH_POINTS_AND_REWARDS extends APPMAKER_WC_REST_Controller
{
    ...
    public function updateCartMeta( $meta ) {
           ...
           $enable_rewards_points = true;
           if ( function_exists('ywpar_get_current_customer') ) {   
                if ( is_user_logged_in() ) {            
                    $ywpar_customer = ywpar_get_current_customer();
                    $enable_rewards_points = ywpar_get_option( 'enable_rewards_points' ) === 'yes' && $ywpar_customer->is_enabled( 'redeem' ) ;
                }else{
                    $enable_rewards_points = false;
                }        
           }         
           if(  $enable_rewards_points ) {
                ob_start();
