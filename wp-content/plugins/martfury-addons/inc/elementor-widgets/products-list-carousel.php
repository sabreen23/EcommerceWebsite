...
class Products_List_Carousel extends Widget_Base {
	...
	protected function render() {
		...
		if ( ! empty( $settings['title'] ) ) {
			$header_html = sprintf( '<div class="cat-header"><%1$s class="cat-title">%2$s</%1$s></div>', \Elementor\Utils::validate_html_tag( $settings['title_size'] ), $settings['title'] );
		}
					...
					'<li class="product">
						<div class="product-item">
							...
							<div class="product-inners">
								<h4 class="woo-loop-product__title">
									<a href="%s">%s</a>
								</h4>
