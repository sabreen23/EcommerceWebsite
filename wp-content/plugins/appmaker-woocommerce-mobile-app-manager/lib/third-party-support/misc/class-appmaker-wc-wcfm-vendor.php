...
class APPMAKER_WC_WCFM_Vendors extends APPMAKER_WC_REST_Posts_Abstract_Controller {
	...
	public function get_product_inquiry( $request ) {
		global $wp, $WCFM;
		$_POST['controller'] = 'wcfm-enquiry-form';
		$_POST['wcfm_enquiry_tab_form'] = array('wcfm_nonce' => wp_create_nonce('wcfm_ajax_nonce') );
		...
		$vendor_info = $this->get_vendor_info( $product_id );
		$vendor_id   = $vendor_info['id'];//print_r($WCFM->wcfm_enquiry->ajax_controller());exit;
		ob_start();
		wp_head();
...
HTML;
	...
	public function get_vendor_listing( $request ) {
		...
		$response              = array();
		//$wcfm_vendors_array    = $WCFM->ajax->wcfm_ajax_controller();// removed as there is user role filter in new wcfm updates
		$wcfm_vendors_array = $WCFMmp->wcfmmp_vendor->wcfmmp_get_vendor_list( true );//print_r($wcfm_vendors_array);exit;
