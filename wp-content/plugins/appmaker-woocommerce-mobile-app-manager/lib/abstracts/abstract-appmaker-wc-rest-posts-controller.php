 ...
 *
 */
abstract class APPMAKER_WC_REST_Posts_Abstract_Controller extends APPMAKER_WC_REST_Controller {
	...
	public function get_item( $request ) {
		...
		$post = get_post( $id );
		if( $this->post_type == 'shop_order' &&  ! APPMAKER_WC::$api->get_settings( 'use_order_number', true ) && class_exists( 'WC_Seq_Order_Number' ) ) {
			$id = WC_Seq_Order_Number::instance()->find_order_by_order_number( $id );
			$post = get_post( $id );
	...
	protected function prepare_items_query( $prepared_args = array(), $request = null ) {
		...
		foreach ( $valid_vars as $var => $index ) {
			if ( isset( $prepared_args[ $var ] ) ) {
				 ...
				 *
				 */
				$query_args[ $var ] = apply_filters( "appmaker_wc_query_var-{$var}", $prepared_args[ $var ] );
			}
		}
	...
	protected function get_allowed_query_vars() {
		...
		if ( current_user_can( $post_type_obj->cap->edit_posts ) ) {
			 ...
			 * `add_filter( 'appmaker_wc_private_query_vars', '__return_empty_array' );`
			 *
			 * @param array $private_query_vars Array of allowed query vars for authorized users.
			 * }
			 */
			$private    = apply_filters( 'appmaker_wc_private_query_vars', $wp->private_query_vars );
		 ...
		 * }
		 */
		$valid_vars = apply_filters( 'appmaker_wc_query_vars', $valid_vars );
